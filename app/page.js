...
function LoadingSpinner() {
  return (
    <div style={{ textAlign: 'center', padding: '48px 0' }}>
      <div style={{ display: 'inline-block', position: 'relative', width: 64, height: 64 }}>
        {[0,1,2,3,4,5,6,7].map(i => (
          <div key={i} style={{
            ...
            background: sectionColors.markers,
            top: '50%', left: '50%',
            transform: `rotate(${i * 45}deg) translate(24px, -50%)`,
            opacity: (i + 1) / 8,
      ...
      <style>{`@keyframes pulse { 0%, 100% { transform: rotate(var(--r)) translate(24px, -50%) scale(1); } 50% { transform: rotate(var(--r)) translate(24px, -50%) scale(1.4); } }`}</style>
    </div>
  );
}
...
function renderContent(content) {
  ...
  while (i < lines.length) {
    ...
    // PCC Marker line with OBSERVED status
    ...
    // OBSERVED/NOT OBSERVED on its own line (following a marker description)
    const statusOnly = line.match(/^(OBSERVED|NOT OBSERVED|INSUFFICIENT EVIDENCE)$/i);
    if (statusOnly) {
      // attach to previous marker if possible, otherwise show inline
      elements.push(
        <div key={i} style={{ marginBottom: 4 }}>
          <MarkerBadge label={statusOnly[1].toUpperCase()} />
        </div>
      );
      i++; continue;
    }

    // Competency heading with score
    const compMatch = line.match(/^(\d+\.\s+.+?)(?:\s*[-–:]\s*Score[:\s]+(\d))?$/i);
    const scoreInline = line.match(/Score[:\s]+(\d)/i);
    if (compMatch && line.match(/^\d+\.\s+[A-Z]/) && !line.startsWith('-')) {
    ...
    if (line.match(/^[A-Z][A-Z\s]+:?$/) && line.length < 60) {
      elements.push(
        <h4 key={i} style={{ color: '#7C5A3A', fontFamily: 'Georgia, serif', fontSize: 14, fontWeight: 700, margin: '16px 0 6px', borderBottom: '1px solid #E8D5BC', paddingBottom: 4 }}>
          {line.replace(/:$/, '')}
        </h4>
      );
      i++; continue;
    ...
    // Heading with colon (like "Key Evidence:", "Growth Opportunity:")
    if (line.match(/^[A-Z][a-zA-Z\s]+:$/) && line.length < 60) {
      elements.push(
        <p key={i} style={{ color: '#7C5A3A', fontSize: 12, fontWeight: 700, margin: '10px 0 2px', textTransform: 'uppercase', letterSpacing: 0.5 }}>
          {line.replace(/:$/, '')}
        </p>
      );
      i++; continue;
    ...
    if (line.startsWith('-') || line.startsWith('•')) {
      elements.push(
        <p key={i} style={{ color: '#6B5744', fontSize: 13, lineHeight: 1.7, paddingLeft: 16, margin: '3px 0', borderLeft: '2px solid #E8D5BC' }}>
          {line.replace(/^[-•]\s*/, '')}
        </p>
      );
      i++; continue;
    ...
    // Numbered item (like "1. Strength name")
    if (line.match(/^\d+\.\s/) && line.length < 100) {
      elements.push(
        <div key={i} style={{ background: '#FDF8F2', border: '1px solid #E8D5BC', borderRadius: 8, padding: '8px 12px', margin: '6px 0' }}>
          <span style={{ color: '#5C4A36', fontSize: 13, fontWeight: 600 }}>{line}</span>
        </div>
      );
      i++; continue;
    }
    ...
    // Default paragraph
    elements.push(
      <p key={i} style={{ color: '#5C4A36', fontSize: 13, lineHeight: 1.75, margin: '4px 0' }}>{line}</p>
    );
    i++;
  }
  return elements;
    ...
    <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #FDF6EE 0%, #F5EDE0 100%)', padding: '40px 20px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <p style={{ color: '#8B7355', fontSize: 16, marginTop: 8 }}>ICF Coaching Session Analyzer · PCC Marker Assessment</p>
        </div>
